...
export function NPCStartDialog({npcId,npcName, onClose}: NPCDialogProps){
    ...
    const messages = [
      `Привет, меня зовут ${npcName}, я помогу тебе с началом игры`,
      `IT-Genetics, игра в которой ты будешь управлять стажером в офисе Chulakov Design и тебе надо стать высококвалифицированным специалистом`,
      `Прокачивай Hard&Soft скиллы, выполняй основные и побочные миссии для продвижения по игре`,
      `Чтобы продолжить нажми 'Играть'`
    ];
